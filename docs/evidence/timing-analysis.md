...
| T8 | Rocket travel time | Frames from a jet firing to the rocket reaching the launcher row, and per-column dwell | Same skill-independence check as T7. Unmeasured; the ROM sets it equal to T7 on the owner's testimony (2026-09-05) that the rocket and the missile travel at the same speed. `tools/probe/rocket-cadence.test.ts` holds the two to one figure off the running machine. |
