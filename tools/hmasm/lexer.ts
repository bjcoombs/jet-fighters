// HMCS40 assembler (PRD R2), stage one: source text -> tokens.
//
...
// hand-written, roughly two thousand words of it, against a 2048-word ceiling and
// 32-word branch pages. A diagnostic that cannot say *where* is close to useless
// at that size, so every token carries a file, a line, a column and a character
// offset, and every node the parser builds keeps the position of the token it
// came from. Nothing downstream ever has to re-scan text to report an error.
//
...
// the shared ISA table in parser.ts. That split is deliberate: the lexer stays
// stable while the architecture is the one in src/machine/cpu/isa.ts.
//
...
// | identifier          | `LAI`, `loop`  | `[A-Za-z_][A-Za-z0-9_]*`                |
