// HMCS40 assembler (PRD R2), the build integration: `import rom from
// '../asm/jetfighter.asm'`.
//
// The game's ROM is source in this repo, not a binary asset - it is written in
// assembly and assembled by tools/hmasm. This plugin makes that fact invisible
// to the application: an `.asm` file is a module that exports a `Uint16Array`,
...
//
// `default` and `rom` are the same `Uint16Array`: `ROM_SIZE` ten-bit words,
// laid out exactly as `Memory` wants them, so `new Memory(rom)` is the whole
// integration. `symbols` carries the label and constant values, which the debug
// UI needs to name an address; `highestAddress` and `ramHighWater` carry the two
// hardware ceilings so a running build can assert on them.
//
...
export function compileAsmModule(path: string): CompiledAsmModule {
  ...
  const code = [
    `// Assembled from ${entry} by tools/hmasm/vite-plugin.ts. Not a file on disk.`,
    `export const rom = new Uint16Array([${image.join(',')}]);`,
 ...
 *
 * export default defineConfig({ plugins: [hmasm()] });
 * ```
 */
export function hmasm(): Plugin {
